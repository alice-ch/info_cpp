...
void Partie::Acquisition(int *x, int *y) {
  cout << "\nX = ";
  if (cin >> *x) {
    cout << "\nY = ";
    if (cin >> *y) {
    } else {
      cout << "\n[Erreur] Veuillez entrer des nombres entiers uniquement.";
      cin.clear();
...
bool Partie::verifCoordonnees(int x, int y) {
  if ((x >= 1) && (x <= 8) && (y >= 1) && (y <= 8)) {
    return true;
  } else {
  ...
  for (int i = 1; i <= 8; i++) {
    for (int j = 1; j <= 8; j++) {
      if (e.getPiece(i, j) != NULL) {
        // On vérifier si la pièce est un roi, si oui on récupère ses
        // coordonnées
...
void Partie::jouerPartie() {
  ...
  changeons_de_piece_a_bouger:
    e.affiche(); // on affiche le plateau à chaque nouvelle action

    /**SAISIE COORDONNEES DE LA PIECE A DEPLACER**/
    if (joueurActuel) {
      cout << "\nAu tour du joueur blanc." << endl
           << "Choisissez les coordonnees de la piece a deplacer.";
    } else {
      cout << "\nAu tour du joueur noir." << endl
           << "Choisissez les coordonnees de la piece a deplacer.";
    }
    ...
    while (!mouvementValide) {
      ...
      if (e.getPiece(xDep, yDep) != NULL) {
        if ((joueurActuel && e.getPiece(xDep, yDep)->isWhite()) ||
            (!joueurActuel && e.getPiece(xDep, yDep)->isBlack())) {
          while (!mouvementValide) {
            cout << "\nChoisissez les coordonnees ou vous souhaitez deplacer "
                    "la piece ou 0 pour quitter : ";
            int x;
            int y;
            Acquisition(&x, &y);
            if ((x == 0) || (y == 0)) {
              goto changeons_de_piece_a_bouger;
            }
            while (!verifCoordonnees(x, y)) {
              Acquisition(&x, &y);
            }
            if (e.getPiece(xDep, yDep)->mouvementValide(e, x, y)) {
              ...
              switch (yaEchec) {
              case 1:
                cout << "\nDeplacement Invalide : Vous etes en Echec si vous "
                        "déplacez cette pièce.";
                break;
              case 2:
          ...
          cout << "\nCette piece n'est pas a vous. Choisissez de nouvelles "
                  "coordonnees :";
        }
      } else {
        cout << "\nIl n'y a pas de piece a la position (" << xDep << "," << yDep
             << ")." << endl
             << "Choisissez de nouvelles coordonnees :";
      }
    }

    joueurActuel = !joueurActuel; // changement de joueur

    // partieFinie = true; //provisoire : afin de ne pas boucler indéfininiment
