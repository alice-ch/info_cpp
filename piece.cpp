...
Piece::~Piece() { cout << "Destructeur Piece" << endl; }
...
int Piece::x() { return m_x; }

int Piece::y() { return m_y; }

bool Piece::isWhite() { return m_white; }

bool Piece::isBlack() { return !m_white; }
...
char Piece::codePiece() { return (m_white) ? 'B' : 'N'; }
...
bool Roi::mouvementValide(Echiquier &e, int x, int y) {
  ...
  if ((abs(x - m_x) <= 1) && (abs(y - m_y) <= 1)) {
    ...
    if (e.getPiece(x, y) != NULL) {
      if (e.getPiece(x, y)->isWhite() !=
          m_white) // Si pièce couleur joueur adverse alors on l'enlève !
      {
        e.enleverPiece(x, y);
...
char Roi::codePiece() { return (m_white) ? 'R' : 'r'; }
...
char Reine::codePiece() { return (m_white) ? 'Q' : 'q'; }
...
bool Fou::mouvementValide(Echiquier &e, int x, int y) {
  ...
  // vérifier si pièce sur chemin et vérifier si pièce sur case d'arrivée et sa
  // couleur
      ...
      while (xDepart != x && yDepart != y) // ET ou OU
      {
        ...
        if (e.getPiece(yDepart, yDepart) != NULL) {
          if (xDepart == x && yDepart == y &&
              e.getPiece(x, y)->isWhite() !=
                  estBlanche) // Si pièce couleur joueur adverse alors on
                              // l'enlève !
          {
            e.enleverPiece(x, y);
            cout << "Le mouvement du Fou est valide : déplacement Haut Droite"
                 << endl;
            estValide = true;
          }
        } else {
      ...
      if (ecartX < 0 && ecartY > 0) // déplacement en bas à gauche
      {
        while (xDepart != x && yDepart != y) // ET ou OU
        {
          ...
          if (e.getPiece(yDepart, yDepart) != NULL) {
            ...
            {
              e.enleverPiece(x, y);
              cout << "Le mouvement du Fou est valide : déplacement Bas Gauche"
                   << endl;
              estValide = true;
            }
          } else {
        ...
        if (ecartX < 0 || ecartY < 0) // déplacement haut gauche
        {
          while (xDepart != x || yDepart != y) // ET ou OU
          {
            ...
            if (e.getPiece(xDepart, yDepart) != NULL) {
              ...
              {
                e.enleverPiece(x, y);
                cout << "Le mouvement du Fou est valide : déplacement Haut "
                        "Gauche"
                     << endl;
                estValide = true;
              }
          ...
          while (ecartX != x && ecartY != y) // ET ou OU
          {
            ...
            if (e.getPiece(xDepart, yDepart) != NULL) {
                                  ...
                                  // l'enlève !
              {
                e.enleverPiece(x, y);
                cout
                    << "Le mouvement du Fou est valide : déplacement Bas Droite"
                    << endl;
                estValide = true;
              }
            } else {
...
char Fou::codePiece() { return (m_white) ? 'F' : 'f'; }
    ...
    if (m_x < x) // déplacement vers droite
    {
      for (i = m_x; i <= x; i++) {
        if (e.getPiece(i, y) != NULL) {
          if (i == x && e.getPiece(i, y)->isWhite() !=
                            m_white) // Si pièce couleur joueur adverse alors on
                                     // l'enlève !
          {
            e.enleverPiece(x, y);
            cout << "Le mouvement de la Tour est valide : deplacement Droite"
                 << endl;
            estValide = true;
          }
        } else {
      ...
      if (m_x > x) // déplacement vers gauche
      {
        for (i = m_x; i <= x; i--) {
          if (e.getPiece(i, y) != NULL) {
            if (i == x && e.getPiece(i, y)->isWhite() !=
                              m_white) // Si pièce couleur joueur adverse alors
                                       // on l'enlève !
            {
              e.enleverPiece(x, y);
              cout << "Le mouvement de la Tour est valide : deplacement Gauche"
                   << endl;
              estValide = true;
            }
          } else {
        ...
        if (m_y < y) // déplacement vers bas
        {
          for (i = m_y; i <= y; i++) {
            if (e.getPiece(x, i) != NULL) {
              if (i == y && e.getPiece(x, i)->isWhite() !=
                                m_white) // Si pièce couleur joueur adverse
                                         // alors on l'enlève !
              {
                e.enleverPiece(x, y);
                cout << "Le mouvement de la Tour est valide : deplacement Bas"
                     << endl;
                estValide = true;
              }
            } else {
        ...
        } else // déplacement vers haut
        {
          for (i = m_y; i <= y; i--) {
            if (e.getPiece(x, i) != NULL) {
              ...
              {
                e.enleverPiece(x, y);
                cout << "Le mouvement de la Tour est valide : deplacement Haut"
                     << endl;
                estValide = true;
              }
            } else {
...
char Tour::codePiece() { return (m_white) ? 'T' : 't'; }

Cavalier::Cavalier(bool white, bool left)
    : Piece(left ? 2 : 7, white ? 1 : 8, white) {
...
char Cavalier::codePiece() { return (m_white) ? 'C' : 'c'; }
...
bool Pion::mouvementValide(Echiquier &e, int x, int y) {
  ...
  if ((m_y == 2 && m_white) || (m_y == 7 && !m_white)) {
    ...
    if ((x == m_x && y == m_y + 2 && m_white) ||
        (x == m_x && y == m_y - 2 && !m_white)) // cas avancer de 2 cases
    {
      if (m_white && e.getPiece(m_x, m_y + 1) == NULL &&
          e.getPiece(m_x, m_y + 2) == NULL) {
      ...
      if ((x == m_x && y == m_y + 1 && m_white) ||
          (x == m_x && y == m_y - 1 && !m_white)) // Cas avancer 1 case
      {
        if (e.getPiece(x, y) == NULL) {
          cout << "Le mouvement du Pion est valide." << endl;
          estValide = true;
        ...
        if ((1 == abs(x - m_x) && y == m_y + 1 && m_white) ||
            (1 == abs(x - m_x) && y == m_y - 1 && !m_white)) {
          if (e.getPiece(x, y) != NULL &&
              e.getPiece(x, y)->isWhite() != m_white) {
            e.enleverPiece(x, y);
            cout << "Le mouvement du Pion est valide." << endl;
            estValide = true;
          }
...
char Pion::codePiece() { return (m_white) ? 'P' : 'p'; }